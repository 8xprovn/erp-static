...
const App = function () {
    ...
    const sidebarSecondaryToggle = function() {
        ...
        // On desktop
        sidebarSecondaryDesktopToggler.off( "click").on('click', function(e) {
            //alert("KO")
            e.preventDefault();
    ...
    // Sidebar navigation
    const navigationSidebar = function(parentDom) {
        ...
        parentDom.find('.' + navClass + ':not(.' + navScrollSpyClass + ')').each(function() {
    ...
    // Navbar navigation
    const navigationNavbar = function(parentDom) {
        ...
        parentDom.find('.dropdown-content a[data-toggle="tab"]').on('click', function() {
            $(this).tab('show');
        });
    };
    ...
    return {
        ...
        initNavigations: function(parentDom) {
            navigationSidebar(parentDom);
            navigationNavbar(parentDom);
        },
        ...
        // Initialize core
        initCore: function(parentDom) {
            App.initBeforeLoad();
            App.initSidebars();
            App.initNavigations(parentDom);
            App.initComponents();
            App.initCardActions();
            App.initDropdowns();
        },
...
$(document).on('DOMContentLoaded',function(e){
    ...
    App.initCore($(e.target));
});
$(document).on('MainContentReloaded',function(){
    App.initReload();
});
